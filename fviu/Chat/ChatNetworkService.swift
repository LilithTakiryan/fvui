...
final class DolaNetworkService: ChatNetworkService {
    ...
    func sendMessage(_ text: String, chatID: String) async throws -> String {
        ...
        logger.info(" Sending message to \(url.absoluteString)")
        ...
        do {
            ...
            logger.info(" Response status: \((response as? HTTPURLResponse)?.statusCode ?? -1)")
            logger.info(" Response data: \(String(data: data, encoding: .utf8) ?? "")")
    ...
    func fetchChats() async throws -> [DolaChatItem] {
        ...
        do {
            ...
            logger.info("Fetched \(chats.count) chats")
            return chats
        } catch {
